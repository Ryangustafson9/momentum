-- 🚀 SPRINT 1: MEMBER DASHBOARD DATABASE SCHEMA
-- Essential tables for member dashboard functionality
...
CREATE TABLE IF NOT EXISTS attendance (
  ...
  status TEXT NOT NULL DEFAULT 'present' CHECK (status IN ('present', 'absent', 'late', 'left_early')),
  check_in_time TIMESTAMP WITH TIME ZONE,
  check_out_time TIMESTAMP WITH TIME ZONE,
  notes TEXT,
  recorded_by UUID REFERENCES auth.users(id), -- Staff member who recorded attendance
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
